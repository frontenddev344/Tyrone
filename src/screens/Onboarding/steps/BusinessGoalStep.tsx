     ...
     <div className="" style={{ boxShadow: '0 2px 8px 0 rgba(16,30,54,.03)' }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-2 sm:px-4 py-6 sm:py-12">
        <div className="w-full max-w-[1133px] mx-auto bg-white rounded-2xl shadow-lg p-4 sm:p-8 lg:p-12 flex flex-col items-center border border-[#F3F4F6]" style={{ borderWidth: '7px', borderRadius: '24px', boxShadow: '0 8px 40px 0 rgba(16,30,54,.10)' }}>
          {/* Icon */}
          <div className="w-[50px] h-[50px] sm:w-[60px] sm:h-[60px] lg:w-[70px] lg:h-[70px] bg-[#32cd32] rounded-full flex items-center justify-center mb-4 sm:mb-6">
          ...
          {/* Heading */}
          <h2 className="text-[22px] sm:text-[28px] lg:text-[32px] font-bold text-[#111827] mb-2 sm:mb-3 text-center">What's your main business goal?</h2>
          {/* Subtitle */}
          <p className="text-[#4B5563] text-[14px] sm:text-[16px] lg:text-[18px] mb-6 sm:mb-8 text-center max-w-[600px] mx-auto" style={{ fontWeight: 400, lineHeight: '1.5' }}>Choose the primary objective that best describes where your business is heading. This helps us recommend the most relevant tools for your journey.</p>
          {/* Goal Cards */}
          <div className="flex flex-col gap-4 sm:gap-6 w-full mb-4 sm:mb-6">
            {goals.map((goal, idx) => (
              <div
                key={goal.title}
                className={`relative flex flex-col sm:flex-row items-start bg-white border rounded-[12px] sm:rounded-[16px] p-4 sm:p-6 lg:p-8 cursor-pointer transition-all duration-150 ${selected === idx ? 'border-[#32cd32] shadow-lg' : 'border-[#E5E7EB]'} min-h-[90px] sm:min-h-[110px] lg:min-h-[120px]`}
                style={{ borderWidth: '2px', boxSizing: 'border-box' }}
                onClick={() => setSelected(idx)}
              >
                {goal.icon}
                <div className="flex-1">
                  <div className="flex items-center mb-2">
                    <span className="font-bold text-[#111827] text-[18px] sm:text-[22px] lg:text-[26px] mr-2" style={{  fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>{goal.title}</span>
                  </div>
                  <div className="text-[14px] sm:text-[16px] lg:text-[19px] text-[#4B5563] mb-2 sm:mb-3 text-left" style={{ lineHeight: '1.4',fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>{goal.desc}</div>
                  <div className="flex flex-wrap gap-2">
                    {goal.tags.map((tag, i) => (
                      <span key={i} className={`px-2 sm:px-3 py-1 rounded-full text-[12px] sm:text-[14px] lg:text-[15px] font-medium ${tag.color}`} style={{ fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>{tag.label}</span>
                    ))}
                  </div>
                </div>
                {/* Custom Checkbox */}
                <div
                  className={`absolute top-4 right-4 sm:top-6 sm:right-6 lg:top-8 lg:right-8 w-4 h-4 sm:w-5 sm:h-5 rounded border-[2px] flex items-center justify-center transition-all duration-150 ${selected === idx ? 'border-[#32cd32] bg-[#32cd32]/10' : 'border-[#D1D5DB] bg-white'}`}
          ...
          {/* Buttons */}
          <div className="flex flex-col sm:flex-row w-full justify-between items-center border-t border-[#F3F4F6] pt-6 sm:pt-8 border-t-[7px] mt-8 sm:mt-16 gap-3 sm:gap-0">
            ...
            <button onClick={onContinue} disabled={selected === null} className="h-12 sm:h-14 px-6 sm:px-10 bg-gradient-to-r from-[#32cd32] to-[#4ade80] font-bold text-[15px] sm:text-[16px] lg:text-[17px] text-white flex items-center justify-center sm:ml-auto transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#32cd32] shadow-none rounded-[12px] sm:rounded-[16px] disabled:opacity-60 disabled:cursor-not-allowed"
